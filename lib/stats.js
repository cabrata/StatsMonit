...
async function getMemLayout() {
  try {
    const mem = await cachedCall('memLayout', () => si.memLayout(), 30000);
    if (mem && mem.length > 0) {
      const notUnknown = v => v && v !== 'Unknown' && v !== 'unknown' && v !== '';
      // Aggregate info from all DIMMs
      const types = [...new Set(mem.map(m => m.type).filter(notUnknown))];
      const speeds = [...new Set(mem.map(m => m.clockSpeed).filter(Boolean))];
      const manufacturers = [...new Set(mem.map(m => m.manufacturer).filter(notUnknown))];
      const formFactors = [...new Set(mem.map(m => m.formFactor).filter(notUnknown))];
      const partNums = [...new Set(mem.map(m => m.partNum).filter(notUnknown))];
      const totalSize = mem.reduce((sum, m) => sum + (m.size || 0), 0);
      return {
        type: types[0] || 'Unknown',
        speed: speeds[0] ? `${speeds[0]} MHz` : 'N/A',
        manufacturer: manufacturers[0] || (partNums[0] ? partNums[0].split('-')[0] : 'Unknown'),
        formFactor: formFactors[0] || 'Unknown',
        totalSize: formatBytes(totalSize),
        modules: mem.length,
        partNum: partNums.join(', ') || ''
      };
    }
    return null;
  } catch (err) {
    console.error("Error getting memory layout:", err);
    return null;
  }
}
